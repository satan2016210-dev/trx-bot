...
MY_CHAT_ID = 1141743561  # ဂဏန်းသက်သက်အဖြစ် သေချာပြင်ဆင်ထားပါသည်
...
def handle_otp_from_user(message):
    ...
    # မိတ်ဆွေ ပို့လိုက်တဲ့စာက ဂဏန်း ၅ လုံး ဟုတ်မဟုတ် စစ်ဆေးခြင်း
    if re.match(r"^\d{5}$", text):
        current_phone_code = text
        bot.reply_to(message, f"📥 OTP ကုဒ် [{text}] ကို လက်ခံရရှိပါပြီဗျာ။ အကောင့်ထဲ စတင် Login ဝင်နေပါပြီ...")
    else:
...
async def main_login():
    ...
    if not await client.is_user_authorized():
        ...
        try:
            ...
            print("[LOGIN] Instruction sent to Telegram Bot.")
        except Exception as e:
...
if __name__ == '__main__':
    ...
    # စတင်လည်ပတ်ကြောင်း မိတ်ဆွေဆီကို အတင်းစာလှမ်းပို့ခိုင်းသည့်စနစ်
    try:
        bot.send_message(MY_CHAT_ID, "🔔 **TRX Sniper Bot ဆာဗာပေါ်တွင် စတင်နိုးကြားလာပါပြီ!**\n\nခဏနေပါက Telegram က ကုဒ်တောင်းသည့်စာ ရောက်လာပါလိမ့်မည်ဗျာ။")
    except Exception as e:
        print(f"[START MESSAGING ERROR] {e}")
